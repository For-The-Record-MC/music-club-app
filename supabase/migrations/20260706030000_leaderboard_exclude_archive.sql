-- Archive albums are cosmetic shelf identity, never competitive credit — but
-- club_leaderboard counted them: every archive album the admin uploaded landed
-- in their albums_chosen (91 "picks"…), and because the archive cycle is
-- created pre-revealed, archive reviews also fed avg_rating_received.
--
-- Re-created with kind <> 'archive' on albums_chosen and avg_rating_received.
-- ratings_given deliberately still counts archive reviews — writing a review
-- is genuine engagement whichever shelf the album sits on. Everything else
-- unchanged.
