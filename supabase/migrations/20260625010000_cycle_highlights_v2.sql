-- Recap v2: extend get_cycle_highlights with the sections powered by the new
-- review/song-note fields (see the notes/reviews overhaul migration):
--
--   • albums[].avg_initial   — average first-listen score, for initial→final
--                              "drift" alongside the existing avg_score.
--   • takes                  — every member's required one-sentence take, per album.
--   • cycle_vibe             — the cycle's most-applied vibe tags (shared song
--                              tags + album-level vibe), best first.
--   • favorite_lyrics        — shared favorite lyrics (per-song + album-level).
--   • best_runs              — the consensus best 3-song run per album (the
--                              most-picked start, its avg run rating, the titles).
--   • most_saved             — tracks most often saved to members' libraries.
--
-- Head-to-head reasons and replayability are collected but intentionally NOT
-- surfaced in the aggregate recap yet. Everything stays member-gated + post-
-- reveal (the guard at the top is unchanged).
...
begin
  ...
  -- Tracks with their resolved track number (for best-run title lookups).
  album_track_nums as (
  ...
  -- ── one-sentence takes (every member, per album) ────────────────────────────
  takes as (
  ...
  -- ── cycle vibe: shared song tags + album-level vibe, most-applied first ──────
  vibe_counts as (
  ...
  -- ── favorite lyrics: shared per-song + album-level ──────────────────────────
  fav_lyrics as (
  ...
  -- ── best 3-song run: most-picked start per album ────────────────────────────
  run_pick as (
  ...
  -- ── most-saved tracks (private flag; counts only, server-side) ───────────────
  saved as (
