-- Recap v4: include each member's email alongside display_name in the recap
-- payload so the client can fall back to the email local-part when someone
-- hasn't set a display name (matches utils/memberName). Full re-create of the
-- function (Postgres requires the whole body); only the four profile-bearing
-- json_build_object calls add 'email', p.email — everything else is v3 verbatim.
